...
def process_with_vad(audio_file):
...
def transcribe_audio(audio_file_path, output_dir="output"):
    ...
    try:
        ...
        # ファイルサイズに関係なくVAD処理を実行（テスト用）
        original_file = audio_file_path
        processed_file = None
        
        print(f"VAD処理を適用します（サイズ: {file_size_mb:.1f}MB）")
        processed_file = process_with_vad(audio_file_path)
        if processed_file != audio_file_path:  # 処理に成功した場合
            audio_file_path = processed_file
            print(f"無音除去処理が完了しました: {audio_file_path}")
            # 処理後のサイズを更新
            file_size_mb = os.path.getsize(audio_file_path) / (1024 * 1024)
            
            # 処理後もサイズが25MBを超える場合の警告
            if file_size_mb > 25:
                print(f"警告: 無音除去後もサイズが25MBを超えています ({file_size_mb:.1f}MB)。APIエラーの可能性があります。")
